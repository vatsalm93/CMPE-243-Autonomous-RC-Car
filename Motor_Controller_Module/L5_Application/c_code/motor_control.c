...
{
    static float pwm_val_motor = 15.6;
    static bool flag = false;
    float speed = get_speed();

    if (speed > (drive->MOTOR_kph + 0.6)){
        setLED(3,1);
        flag = stop_motor();
        pwm_val_motor = STOP;
    }
    else {
        if (pwm_val_motor < STOP) {
            pwm_val_motor = STOP;
        }
        else if (pwm_val_motor > SPEED_LIMIT) {
            pwm_val_motor = SPEED_LIMIT;
        }

        if (speed < (drive->MOTOR_kph) && pwm_val_motor < SPEED_LIMIT) {
            if(flag == true){
                pwm_val_motor = 15.7;
               // u0_dbg_printf("flag: %d\n", flag);
                flag = false;
            }
...
//        else if (speed > drive->MOTOR_kph + 1) {
//            pwm_val_motor = 15.3;
//            pwm_val_motor -= STEP_DOWNHILL;
//            set_pwm_value(motor_1, pwm_val_motor);
//        }
        else {
            set_pwm_value(motor_1, pwm_val_motor);
