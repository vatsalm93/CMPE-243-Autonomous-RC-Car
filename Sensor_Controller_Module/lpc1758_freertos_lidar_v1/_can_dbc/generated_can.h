...
// static const dbc_msg_hdr_t CAR_CONTROL_HDR =                      {  104, 3 };
static const dbc_msg_hdr_t SENSOR_NODE_HDR =                      {  105, 8 };
// static const dbc_msg_hdr_t MOTOR_NODE_HDR =                       {  106, 2 };
...
static const dbc_msg_hdr_t SENSOR_DEBUG_HDR =                     {  115, 1 };
// static const dbc_msg_hdr_t MOTOR_DEBUG_HDR =                      {  116, 2 };
// static const dbc_msg_hdr_t MASTER_DEBUG_HDR =                     {  117, 1 };
// static const dbc_msg_hdr_t GPS_DEBUG_HDR =                        {  118, 1 };
// static const dbc_msg_hdr_t BRIDGE_DEBUG_HDR =                     {  119, 1 };




/// Message: SENSOR_NODE from 'SENSOR', DLC: 8 byte(s), MID: 105
typedef struct {
    uint16_t SENSOR_FRONT_cm;                 ///< B9:0  Min: 0 Max: 645   Destination: MASTER,MOTOR,BRIDGE
    uint8_t LIDAR_Obstacle_FRONT : 3;         ///< B12:10  Min: 0 Max: 7   Destination: MASTER,MOTOR,BRIDGE
    uint8_t LIDAR_Obstacle_RIGHT : 3;         ///< B15:13  Min: 0 Max: 7   Destination: MASTER,MOTOR,BRIDGE
    uint8_t LIDAR_Obstacle_LEFT : 3;          ///< B18:16  Min: 0 Max: 7   Destination: MASTER,MOTOR,BRIDGE
    uint8_t LIDAR_Obstacle_BACK : 3;          ///< B21:19  Min: 0 Max: 7   Destination: MASTER,MOTOR,BRIDGE
...
/// Message: SENSOR_DEBUG from 'SENSOR', DLC: 1 byte(s), MID: 115
typedef struct {
    uint8_t IO_DEBUG_CAN_init : 1;            ///< B0:0   Destination: DBG
    uint8_t IO_DEBUG_sensor_init : 1;         ///< B1:1   Destination: DBG
    uint8_t IO_DEBUG_CAN_TX : 1;              ///< B2:2   Destination: DBG
    uint8_t IO_DEBUG_bus_off : 1;             ///< B3:3   Destination: DBG
...
/// Encode SENSOR's 'SENSOR_DEBUG' message
/// @returns the message header of this message
static inline dbc_msg_hdr_t dbc_encode_SENSOR_DEBUG(uint8_t bytes[8], SENSOR_DEBUG_t *from)
{
    uint32_t raw;
    bytes[0]=bytes[1]=bytes[2]=bytes[3]=bytes[4]=bytes[5]=bytes[6]=bytes[7]=0;

    raw = ((uint32_t)(((from->IO_DEBUG_CAN_init)))) & 0x01;
    bytes[0] |= (((uint8_t)(raw) & 0x01)); ///< 1 bit(s) starting from B0

    raw = ((uint32_t)(((from->IO_DEBUG_sensor_init)))) & 0x01;
    bytes[0] |= (((uint8_t)(raw) & 0x01) << 1); ///< 1 bit(s) starting from B1

    raw = ((uint32_t)(((from->IO_DEBUG_CAN_TX)))) & 0x01;
    bytes[0] |= (((uint8_t)(raw) & 0x01) << 2); ///< 1 bit(s) starting from B2

    raw = ((uint32_t)(((from->IO_DEBUG_bus_off)))) & 0x01;
    bytes[0] |= (((uint8_t)(raw) & 0x01) << 3); ///< 1 bit(s) starting from B3

    return SENSOR_DEBUG_HDR;
}

/// Encode and send for dbc_encode_SENSOR_DEBUG() message
static inline bool dbc_encode_and_send_SENSOR_DEBUG(SENSOR_DEBUG_t *from)
{
    uint8_t bytes[8];
    const dbc_msg_hdr_t hdr = dbc_encode_SENSOR_DEBUG(bytes, from);
    return dbc_app_send_can_msg(hdr.mid, hdr.dlc, bytes);
}



/// Not generating code for dbc_encode_MOTOR_DEBUG() since the sender is MOTOR and we are SENSOR

/// Not generating code for dbc_encode_MASTER_DEBUG() since the sender is MASTER and we are SENSOR

/// Not generating code for dbc_encode_GPS_DEBUG() since the sender is GPS and we are SENSOR

/// Not generating code for dbc_encode_BRIDGE_DEBUG() since the sender is BRIDGE and we are SENSOR
...
/// Not generating code for dbc_decode_SENSOR_DEBUG() since 'SENSOR' is not the recipient of any of the signals

/// Not generating code for dbc_decode_MOTOR_DEBUG() since 'SENSOR' is not the recipient of any of the signals

/// Not generating code for dbc_decode_MASTER_DEBUG() since 'SENSOR' is not the recipient of any of the signals

/// Not generating code for dbc_decode_GPS_DEBUG() since 'SENSOR' is not the recipient of any of the signals

/// Not generating code for dbc_decode_BRIDGE_DEBUG() since 'SENSOR' is not the recipient of any of the signals
